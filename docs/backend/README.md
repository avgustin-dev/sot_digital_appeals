...
SMS не используется. Уведомление пишется в карточку (`notifications[]`, канал `email` если гражданин указал почту, иначе `system`) и показывается на талоне и при проверке статуса по коду на публичном сайте. Реальную отправку письма делает бэкенд.
